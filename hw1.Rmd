...
The deviance for deviance for a logistic regression model (bernoulli distribution, logit link) is given by the following formula,

$$ D = \text{sign}(Y_i-\hat{p_i}) ~ 2 \sum_{i=1}^n \left(Y_i \log \frac{Y_i}{\hat{p}_i}+(1-Y_i)\log \frac{1-Y_i}{1 - \hat{p}_i}\right)  $$
using the definition of deviance from class, derive this formula starting from the Bernoulli likelihood.
